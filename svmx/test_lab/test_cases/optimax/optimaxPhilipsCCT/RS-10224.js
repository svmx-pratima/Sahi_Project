/*Check if the unavailability is within 
 the dispatch process threshold duration of the workorder. 
 i.e. if the information for unavailability 
 is known within the dispatch threshold window 
 */

/*Validating WO reassignment to another Tech */

//Create a WO and assign the AB. Further, on assignment of WO, create a event on tech calendar.wait for 5 min and see if the reassignment of Tech has happened.
...
if(($ReassignedServiceTech!=$InitialServiceTech)&&($SchedulingStatus=="Tentative")){
	_log("Due to non availablity of Tech and since the WO had not reached dispatch threshould, a new tech has been assigned \n"+$ReassignedServiceTech+"Previous Tech: \n"+$InitialServiceTech);
}
else if(($ReassignedServiceTech==$InitialServiceTech)&&($SchedulingStatus=="Tentative")){
	_log("Technician is available for the schedule start time.");
}
